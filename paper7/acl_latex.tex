...
We introduce \textbf{GEM} (General Elegant Mathematics), a benchmark of 240 problems where optimal solutions require key insights such as symmetry exploitation, geometric visualization, or variable substitution, while brute-force computation leads to either errors or excessive complexity.
...
\section{The GEM Benchmark}
\label{sec:benchmark}

We construct the \textbf{General Elegant Mathematics (GEM)} benchmark to systematically evaluate whether LLMs can discover and apply elegant problem-solving strategies.

\subsection{Design Principles}

Each problem in GEM satisfies the following criteria:
...
GEM contains 240 problems across five categories:
...
\item Success rate improves by 23\% on GEM test set
...
- \textbf{Novel GEM variants}: 18\% improvement when problem parameters are varied
...
Future work should:
(1) Extend GEM to other domains (programming, physics problems, logical puzzles),
...
Through the GEM benchmark and comprehensive evaluation of 12 LLMs, we demonstrate that this phenomenon is robust across model families and problem categories.
...
GEM focuses on mathematical reasoning and may not generalize to all domains requiring strategic insight (e.g., creative writing, open-ended problem solving).
...
We do not include systematic human evaluation on GEM, making it difficult to assess whether models are approaching, exceeding, or falling short of human expert performance.


\section*{Ethics Statement}

\paragraph{Responsible Benchmarking.}
GEM is designed to evaluate reasoning quality, not to be ``gamed'' by models.
...
We will release GEM under an open license to ensure broad access for academic research, though we note that evaluating large proprietary models may require significant compute resources or API costs.

\paragraph{Misuse Potential.}
GEM could be misused to create adversarial inputs that expose LLM weaknesses.
...
\section{GEM Example Problems}
\label{appendix:examples}

We provide complete examples from each category in GEM:
...
\begin{itemize}[leftmargin=*, itemsep=0pt]
\item Complete GEM benchmark with all 240 problems and solutions
